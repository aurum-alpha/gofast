...
- **Channel attributes** (`internal/channelattr`): SQLite `{data_dir}/channelattr/attr.db` holds **current + history** labels keyed by `(provider, channel_id, kind)`. Generations are the atomic emit trick for playlist/guide/raw — they are not a ledger of “what changed.” Labels that evolve over time use this one shared store.
  ...
  **Reading history:** per-channel `History` stays for health UI; presence history is `GET /api/channels/{provider}/{id}/presence/history`. Cross-channel `EventsSince(since, kinds…)` lists `presence` + `classification` (or caller-specified kinds) since a timestamp, oldest→newest, bounded — the contract for overnight digests (daily email) and `GET /api/presence/summary` (Absent now + Dropped/Added 7d). Classification still appends only when the stored class changes.
