...
- **Health probes (`internal/health`):** L1 is the existing classifier at refresh. L2 (default 24h) GET first media segment for **NATIVE only** (never scheduled on BEACON): prefer ranged GET, retry without `Range` on HTTP 416. Each L2 check records `http_status` on the health attr (success or failure). Timing is **last sweep + interval** (persisted in `{data_dir}/channelattr/health_schedule.json`), not process start — cold start / overdue uses a short settle delay. L3 ffprobe is **off by default** (`health.l3_enabled` / `FASTGEN_HEALTH_L3_ENABLED`); when on, same last+interval anchoring; BEACON probes use the emitted proxy URL. Results `EmitCheck` into the channel-attr bus. `exclude_unhealthy` (default false) prunes `HealthDown` from export. Prometheus `gofast_provider_channels_health{status=…}`. `GET /api/health/schedule` exposes last/next L2/L3.
