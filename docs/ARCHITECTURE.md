...
- **Health probes (`internal/health`):** L1 is the existing classifier at refresh. L2 (default 24h) ranged-GET first media segment for **NATIVE only** (never scheduled on BEACON). L3 ffprobe is **off by default** (`health.l3_enabled` / `FASTGEN_HEALTH_L3_ENABLED`); when on, BEACON probes use the emitted proxy URL. Results `EmitCheck` into the channel-attr bus. `exclude_unhealthy` (default false) prunes `HealthDown` from export. Prometheus `gofast_provider_channels_health{status=…}`.
