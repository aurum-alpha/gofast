...
| `cmd/fastproxy` | `fastproxy` | Add-on: Class A HLS rewrite / dialect resolve; Class B `/stable/` demux-stable encode |
...
- **ffmpeg/ffprobe:** shipped in **both** images (`debian:bookworm-slim` + ffmpeg). Gen uses ffprobe for Health L2 (`health.ffprobe_path`). Proxy uses ffmpeg for Class B `/stable/` demux-stable encode pipes (`FASTPROXY_FFMPEG`, `FASTPROXY_DEMUX_STABLE_MAX`, `FASTPROXY_DEMUX_STABLE_SIZE`).
- **Optional YAML** on the data volume: `/data/config.yaml` for structured, non-secret settings. Provider *implementations* are code (packages under `internal/provider/<id>` exposing `New` + `DefaultSettings`, wired into a `map[id]provider.Reader`); the `providers` block only *overlays settings* for a known provider and cannot add one without shipping Go (unknown ids are ignored/warned). Every provider — LG, MJH providers (Pluto, Samsung, Roku, Plex), published-pair providers (Xumo, Tubi, TCL, LocalNow), and DistroTV (jsrdn feed) — runs only when its YAML block is present and may be disabled explicitly with `enabled: false`. A generated defaults-only config enables nothing. Runtime data only — ship `config.example.yaml` as a template; never commit a filled production file.
...
- **Per-channel emit (`providers.<id>.channel_emit`):** map of `ChannelEmit` keyed by normalized id on `ProviderSettings` — customizes emitted name/group/number/logo and export mode, plus `dedupe: true` when Dedupes apply disabled the channel. Applied in `refresh` transform/prepare (presentation → groups → pre-export soft-clear → emission policy); marshalers use `DisplayName()` / `EmittedGroup` / `OffsetNumber` / `LogoURL`. Persisted via `PUT /api/channels/{provider}/{id}/emit` as a whole-map `Store.Save` PathOp (normalized ids may contain `.`). Channel detail paints configured `emit` + `emit_defaults` for the Customize checkboxes and never invents `dedupe` (preserves an existing Dedupes marker). Force-include sets `ForceInclude` so `exclude_unhealthy` is skipped; DRM / needs-proxy / missing id|stream / unsupported classification remain hard blocks. `Channel.FilterReasons []FilterReason` accumulates every applicable exclusion; primary `FilterReason` drives badges. `EmittedURL` is minted whenever a playback path exists even if the channel stays excluded. Raw cache and synthetic-number maps are never mutated.
...
- **Generated artifacts** live under `{data_dir}/cache/`, owned entirely by `internal/cache` (the only package that touches generation disk). Each provider uses immutable generations selected by an atomic `current` pointer. A generation contains `playlist.m3u`, `guide.xml`, lean `meta.json`, and exact upstream payloads under `raw/` (`schedule.json` for LG; `channels.json.gz` + `guide.xml.gz` for MJH; `playlist.m3u` + `guide.xml[.gz]` for published pairs). `meta.json` retains fetched time and historical synthetic channel-number assignments; health, classification, and presence live in the channel-attr store. Older generations may still carry a `classifications` map — boot seeds missing attr rows from it once, then stops writing the map. `status.json` remains outside the generation. The combined aggregate uses the same generation/`current` model under `aggregate/` (`playlist.m3u` + `epg.xml` as one pair). Legacy root-level aggregate files remain readable until the next rebuild. Emitters are deterministic: channels are globally sorted by positive final number with unnumbered rows last, aggregate ids are provider-namespaced, and XMLTV is re-parsed plus semantically checked before publication. `<lcn>` is an intentional non-DTD compatibility extension; Jellyfin numbering comes from M3U `tvg-chno`. On boot fastgen reparses the selected raw files, restores irreproducible metadata, regenerates outputs under the current emission config, and resumes the refresh interval without a network call. An empty aggregate rebuild (no exportable channels) leaves the prior aggregate generation untouched. Keep the cache on a persistent volume/bind mount. **Cache management:** `GET /api/cache` inventories disk sizes + channelattr row counts; soft purge (`POST …/cache/purge`) never deletes the serving generation; `DELETE /api/logos…` clears logos (next GET refills lazily); orphan sweep removes staging leftovers, gens beyond current+1, stale/out-of-lineup logos, and unconfigured provider dirs. Channelattr is never deleted by these actions.
- **Channel attributes** (`internal/channelattr`): SQLite `{data_dir}/channelattr/attr.db` holds **current + history** labels keyed by `(provider, channel_id, kind)`. Generations are the atomic emit trick for playlist/guide/raw — they are not a ledger of “what changed.” Labels that evolve over time use this one shared store.

  Three kinds answer three questions: **`health`** — does it play right now?; **`classification`** — what dialect / export path?; **`presence`** — is this normalized id still in the provider’s **catalog** (the lineup after `prepare`), regardless of Dedupes / emit-disable / DRM keeping it out of the M3U?

  **Presence recording:** after a successful `prepare` (network refresh or warm restore/reapply), refresh diffs the new catalog ID set against Current presence with `state=present` — not against the in-memory `Feed` (empty or stale across reboot). New IDs append `present` (display name + tvg-id in the value for readable digests); IDs that were present and are gone append `absent` (last known name kept). Unchanged IDs are silent. First run / upgrade with no presence rows bootstraps the whole catalog as `present` once. Writers use the same bus as health/classification (`Emit` → AttrReceiver); boot Restore may `Handle` directly before `Receive`. Source is `refresh` or `restore`. Presence is **not** logo/URL churn, export/dedupe toggles, or multi-replica exactly-once.

  **Reading history:** per-channel `History` stays for health UI; presence history is `GET /api/channels/{provider}/{id}/presence/history`. Cross-channel `EventsSince(since, kinds…)` lists `presence` + `classification` (or caller-specified kinds) since a timestamp, oldest→newest, bounded — the contract for overnight digests (daily email) and `GET /api/presence/summary` (Absent now + Dropped/Added 7d). Classification still appends only when the stored class changes. Ops-report compose issues **separate** `EventsSince` calls per kind (presence / classification / health) so high-volume health probes cannot starve other deltas; health rows in the digest are **status transitions** only.

  **UI ghosts:** channels with Current `presence=absent` are synthesized into `GET /api/channels` (and detail/emit GET) so operators can filter Status → **Absent** and open history after the id left the live feed. Ghosts carry `FilterReason` `absent from provider` and are not exportable. Full Channels Status vocabulary (In lineup, Via proxy, Duplicate, Absent, …) is documented in SPEC → Web UI.

  Producers `Emit` on a Go channel; one AttrReceiver goroutine is the sole writer (boot may `Handle` once to seed legacy meta classifications before Receive starts). Boot `LoadCurrent` is O(attrs), not O(history). Refresh/restore **Annotate** paints current health/classification onto `Channel` before `Feed.Set` (classification only when the in-memory field is empty; presence is not painted onto live channels — only ghosts set `presence=absent`). Event rows older than **90 days** are pruned (throttled on write); **current** rows are never deleted by that prune. Soft cache purge / orphan sweep still never deletes `attr.db`.
- **Ops report (`internal/opsreport`):** daily email scheduler (config Reloader like health). Persists `{data_dir}/ops_reports/state.json` (last official success, next fire, durable refresh tallies since last official) and archives full sends as `report-*.json` (structured payload + rendered subject/text/html; 90-day prune). Official fire resets tallies; preview/test do not. SMTP via stdlib `net/smtp` + STARTTLS; password from YAML or `FASTGEN_SMTP_PASSWORD` (env wins). APIs: `GET /api/ops-report/schedule`, archives list/get/resend, `POST …/test-smtp`, `POST …/send-preview` (same-origin on mutating routes).
...
- **Published-pair validation:** Xumo, Tubi, TCL, and LocalNow parse shared EXTINF/XMLTV inputs, sanitize upstream bare ampersands, normalize joins, and log playlist-to-guide ID match rates. DistroTV uses Distro’s jsrdn JSON feed (opaque catalog URLs; FASTProxy `DISTRO_RESOLVE` at tune-in). Numberless channels receive stable first-seen assignments from each provider's configured synthesis base; removed IDs remain reserved and playlist order never determines identity.
...
internal/
  ...
  classifier/
  channelattr/ # current+history channel labels (health, classification, presence)
  health/     # EmitCheck, Health L1 segment / Health L2 ffprobe, scheduler
  opsreport/  # daily email digest (schedule, SMTP, archive)
  logocache/ refresh/
...
- Ops-report state/archives under `/data/ops_reports/` (not under `channelattr/`).
